...
!(async () => {
  ...
  message += "<font color=\'#FFA500\'>[通知] </font><font color=\'#006400\' size='3'>康康和锟锟的京喜工厂</font> \n\n --- \n\n"
  await requestAlgo();
          ...
          if (safeGet(data)) {
            ...
            if (data['ret'] === 0) {
              ...
              if (data.factoryList && data.productionList) {
                ...
                message += "<font color=\'#778899\' size=2>" + `【京东账号${$.index}】${$.nickName}\n` + "</font>\n\n"
                message += "<font color=\'#778899\' size=2>" + `【生产商品】${$.productName}\n` + "</font>\n\n";
                message += "<font color=\'#778899\' size=2>" + `【当前等级】${data.user.userIdentity} ${data.user.currentLevel}\n` + "</font>\n\n";
                message += "<font color=\'#778899\' size=2>" + `【生产进度】${((production.investedElectric / production.needElectric) * 100).toFixed(2)}%\n` + "</font>\n\n";
                ...
                if (!data.factoryList) {
                  ...
                  message += "<font color=\'#778899\' size=2>" + `【提示】京东账号${$.index}[${$.nickName}]京喜工厂活动未开始\n请手动去京东APP->游戏与互动->查看更多->京喜工厂 开启活动\n` + "</font>\n\n"
                  // $.msg($.name, '【提示】', `京东账号${$.index}[${$.nickName}]京喜工厂活动未开始\n请手动去京东APP->游戏与互动->查看更多->京喜工厂 开启活动`);
                } else if (data.factoryList && !data.productionList) {
                  console.log(`【提示】京东账号${$.index}[${$.nickName}]京喜工厂未选购商品\n请手动去京东APP->游戏与互动->查看更多->京喜工厂 选购\n`)
                  message += "<font color=\'#778899\' size=2>" + `【提示】京东账号${$.index}[${$.nickName}]京喜工厂未选购商品\n请手动去京东APP->游戏与互动->查看更多->京喜工厂 选购\n` + "</font>\n\n"
  ...
  if (exchangeEndTime) {
    ...
    if (nowTimes < exchangeEndTime) {
      ...
      if ((exchangeEndTime - nowTimes.getTime()) <= 3600000 * 4) {
        ...
        message += "<font color=\'#778899\' size=2>" + `【京东账号${$.index}】${$.nickName}\n【生产商品】${$.productName}${expiredTime}小时后兑换超时\n【兑换截止时间】${$.exchangeEndTime}\n请速去京喜APP->首页->好物0元造进行兑换`, {'open-url': jxOpenUrl, 'media-url': $.picture} + "</font>\n\n"
...
async function showMsg() {
  return new Promise(async resolve => {
    message +="<font color=\'#778899\' size=2>" + `【收取自己零件】${$.pickUpMyselfComponent ? `获得${$.pickEle}电力` : `今日已达上限`}\n` + "</font>\n\n";
    message += "<font color=\'#778899\' size=2>" +`【收取好友零件】${$.pickUpMyselfComponent ? `获得${$.pickFriendEle}电力` : `今日已达上限`}\n` + "</font>\n\n";
...
function getHelp() {
        if($.encryptPin){
    		...
    		return new Promise(resolve => {
    			$.get({
    				url: "http://api.tyh52.com/act/get/dreamFactory/2"
    			}, (err, resp, data) => {
    				try {
    					if (data) {
    						data = JSON.parse(data);
	...
	function setHelp() {
	    if($.encryptPin){
	        return new Promise(resolve => {
			$.get({
				url: "http://api.tyh52.com/act/set/dreamFactory/" + $.encryptPin
			}, (err, resp, data) => {
				try {
					if (data) {
						data = JSON.parse(data);
        ...
        if (err) {
          ...
          if (data) {
            ...
            if (data['status'] === 200) {
              ...
              if (enCryptMethodJDString) eval(enCryptMethodJDString+";$.enCryptMethodJD=test");
