...
let message = "<font color=\'#FFA500\'>[通知] </font><font color=\'#006400\' size='3'>康康和锟锟相爱的记录</font> \n\n --- \n\n" , subTitle = '', option = {}, isFruitFinished = false;
const retainWater = 100;//保留水滴大于多少g,默认100g;
let jdNotify = false;
let jdFruitBeanCard = false;
...
async function jdFruit() {
    ...
    try {
        ...
        if ($.farmInfo.farmUserPro) {
            ...
            message = message +  `【水果名称】${$.farmInfo.farmUserPro.name}\n`;
            ...
            if ($.farmInfo.treeState === 2 || $.farmInfo.treeState === 3) {
                ...
                fruitName = `${$.farmInfo.farmUserPro.name}`
...
async function doDailyTask() {
    ...
    await getAwardInviteFriend();//邀请好友
...
async function predictionFruit() {
    ...
    s = `【预测】${waterD === 1 ? '明天' : waterD === 2 ? '后天' : waterD + '天之后'}(${timeFormat(24 * 60 * 60 * 1000 * waterD + Date.now())}日)可兑换水果🍉`
    message += "<font color='#BA55D3' size=2> " + s  + "</font> \n\n"
}
//浇水十次
...
async function masterHelpShare() {
    ...
    if (helpSuccessPeoples && helpSuccessPeoples.length > 0) {
        message += "<font color=\\'#778899\\' size=2>【您助力的好友👬】" + `${helpSuccessPeoples.substr(0, helpSuccessPeoples.length - 1)}` + "</font>\n\n";
    }
    if (salveHelpAddWater > 0) {
        // message += `【助力好友👬】获得${salveHelpAddWater}g💧\n`;
        that.log(`【助力好友👬】获得${salveHelpAddWater}g💧\n`);
    }
    message += "<font color=\\'#778899\\' size=2>" + "【今日剩余助力👬】" + `${remainTimes}` +"次</font>";
